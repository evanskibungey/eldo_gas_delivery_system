...
interface Props {
    item: {
        id: number; group_id: number; name: string; description: string | null;
        price: number; photo_url: string | null; sort_order: number; is_active: boolean;
    };
...
export default function EditItem({ item, sizes }: Props) {
    ...
    const [photoPreview, setPreview]= useState<string | null>(item.photo_url);
    ...
    function onSubmit(e: React.FormEvent) {
        ...
        router.post(`/admin/catalogue/addon-items/${item.id}`, fd, {
    ...
    return (
        <AdminLayout title={`Edit Item â€” ${item.name}`} subtitle="Update add-on item details">
            <div className="mb-6">
                        ...
                        <form onSubmit={onSubmit} className="space-y-5">
                            ...
                            <div>
                                ...
                                <select
                                    ...
                                    className="mt-1.5 h-10 w-full rounded-md border border-slate-200 bg-slate-50 px-3 text-sm text-slate-800 focus:border-orange-400 focus:outline-none focus:ring-2 focus:ring-orange-400/20 focus:bg-white transition-all"
                                >
                                    {allGroups.map(g => (
